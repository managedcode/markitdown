...
/// </summary>
public sealed class ImageConverter : DocumentPipelineConverterBase
{
    ...
    public override async Task<DocumentConverterResult> ConvertAsync(Stream stream, StreamInfo streamInfo, CancellationToken cancellationToken = default)
    {
        ...
        var imageBytes = await File.ReadAllBytesAsync(source.FilePath, cancellationToken).ConfigureAwait(false);
        ...
        {
            var description = await SafeDescribeAsync(imageBytes, streamInfo, cancellationToken).ConfigureAwait(false);
    ...
    private static string ResolveDefaultExtension(StreamInfo streamInfo)
        => string.IsNullOrWhiteSpace(streamInfo.Extension) ? ".png" : streamInfo.Extension!;

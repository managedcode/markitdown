...
/// </summary>
public sealed class RssFeedConverter : DocumentConverterBase
{
    public RssFeedConverter()
        : base(priority: 120) // Higher priority than general XML converter for feeds
    {
    }
    ...
    private static readonly IReadOnlyList<string> AcceptedMimeTypePrefixes = new List<string>
    {
        MimeHelper.RSS,
        MimeHelper.ATOM,
        MimeHelper.RDF,
    ...
    public override bool AcceptsInput(StreamInfo streamInfo)
    {
        ...
        {
            if (normalizedMime is not null && normalizedMime.StartsWith(prefix, StringComparison.OrdinalIgnoreCase))
                return true;

            if (streamInfo.MimeType is not null && streamInfo.MimeType.StartsWith(prefix, StringComparison.OrdinalIgnoreCase))
                return true;
        }
    ...
    public override bool Accepts(Stream stream, StreamInfo streamInfo, CancellationToken cancellationToken = default)
    {
        ...
        var normalizedMime = streamInfo.ResolveMimeType();
        if (extension == ".xml" ||
            string.Equals(normalizedMime, MimeHelper.XML, StringComparison.OrdinalIgnoreCase) ||
            string.Equals(normalizedMime, "application/xml", StringComparison.OrdinalIgnoreCase))
        {

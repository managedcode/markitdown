...
public sealed class BingSerpConverter : DocumentConverterBase
{
    ...
    private static string? TryDecodeBase64Url(string value)
    {
        ...
        {
            RecordDecodeFailure(value.Length, ex);
            return null;
        }
    }

    private static void RecordDecodeFailure(int length, Exception exception)
    {
        const string decodeFailureDescription = "Failed to decode Bing SERP payload.";
        var current = Activity.Current;
        if (current is not null)
        {
            current.SetTag(TelemetryTags.Converter, "BingSerp");
            current.AddEvent(new ActivityEvent(
                "BingSerpConverter.DecodeFailed",
                tags: new ActivityTagsCollection
                {
                    { TelemetryTags.Converter, "BingSerp" },
                    { TelemetryTags.Base64Length, length },
                    { TelemetryTags.ExceptionType, exception.GetType().FullName ?? "unknown" },
                    { TelemetryTags.ExceptionMessage, exception.Message }
                }));
            current.SetTag(TelemetryTags.ActivityStatusDescription, decodeFailureDescription);
            current.SetStatus(ActivityStatusCode.Error, decodeFailureDescription);
            return;
        }

        if (!MarkItDownDiagnostics.DefaultActivitySource.HasListeners())
        {
            return;
        }

        using var activity = MarkItDownDiagnostics.DefaultActivitySource.StartActivity("BingSerpConverter.DecodeFailed", ActivityKind.Internal);
        if (activity is null)
        {
            return;
        }

        activity.SetTag(TelemetryTags.Converter, "BingSerp");
        activity.SetTag(TelemetryTags.Base64Length, length);

...
public sealed class StreamInfo
{
    ...
    /// <summary>
    /// Resolve the most appropriate MIME type for this stream info using the known metadata and <see cref="MimeHelper"/>.
    /// </summary>
    public string? ResolveMimeType()
    {
        ...
        {
            var candidate = MimeHelper.GetMimeType(LocalPath);
            if (!IsUnknownMime(candidate))
            {
                return candidate;
            }
    ...
    /// <summary>
    /// Determine whether the supplied MIME string matches any of the provided prefixes.
    /// </summary>
    public static bool MatchesMime(string? mime, IEnumerable<string> prefixes)
    {
        ...
        foreach (var prefix in prefixes)
        {
            if (!string.IsNullOrWhiteSpace(prefix) &&
                mime!.StartsWith(prefix, StringComparison.OrdinalIgnoreCase))
            {
                return true;
            }
        }
    ...
    /// <summary>
    /// Determine whether the resolved MIME type of this stream info matches any of the provided prefixes.
    /// </summary>
    public bool MatchesMime(IEnumerable<string> prefixes)
        => MatchesMime(ResolveMimeType(), prefixes);
    ...
    private static bool IsUnknownMime(string? mime)
        => string.IsNullOrWhiteSpace(mime) || string.Equals(mime, MimeHelper.BIN, StringComparison.OrdinalIgnoreCase);
}
